        ...
        subBut:function(){
            ...
            if( that.fpg != "" ){
                 if( !(/^\d+(\.\d+)?$/.test(that.fpg)) ){
                    mui.toast('请填写正确FPG',{ duration:'long', type:'div' });
                    return false; 
                }
            }
            if( that.HbAIc != "" ){
                  if( !(/^\d+(\.\d+)?$/.test(that.HbAIc)) ){
                    mui.toast('请填写正确HbAIc',{ duration:'long', type:'div' });
                    return false; 
                }
            }
            if( that.fins != "" ){
                 if( !(/^\d+(\.\d+)?$/.test(that.fins)) ){
                    mui.toast('请填写正确FINS',{ duration:'long', type:'div' });
                    return false; 
                }
            }
            if( that.ast != "" ){
                 if( !(/^\d+(\.\d+)?$/.test(that.ast)) ){
                    mui.toast('请填写正确AST',{ duration:'long', type:'div' });
                    return false; 
                }
            }
            if( that.alt != "" ){
                if( !(/^\d+(\.\d+)?$/.test(that.alt)) ){
                    mui.toast('请填写正确ALT',{ duration:'long', type:'div' });
                    return false; 
                }
            }
            ...
            // }
            // if( !(/^\d+(\.\d+)?$/.test(that.lh)) ){
            //     mui.toast('请填写正确LH/FSH',{ duration:'long', type:'div' });
            //     return false; 
            // }
            // if( !(/^\d+(\.\d+)?$/.test(that.fins)) ){
            //     mui.toast('请填写正确FINS',{ duration:'long', type:'div' });
