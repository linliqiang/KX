...
// 普通提示弹框
var my_setTimeout_m;
function my_alert_m(msg){
  // 先清空时间函数，和已有的弹框
  if(my_setTimeout_m){
    clearTimeout(my_setTimeout_m);
  }
  $("body .my_alert_m").remove();
  $("body").append("<div class='my_alert_m'><span>"+msg+"</span></div>");
...
// 移动端下拉列表没有任何初始数据
function dataErr(nodeClass,msg){
    $(nodeClass).append("<div class='noData'><span>"+msg+"</span></div>")
}
